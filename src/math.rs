...
impl AddAssign for Tangent {
	fn add_assign(&mut self, rhs :Self){
		*self = Bitangent{ tangent:((self.tangent.0 + rhs.tangent.0),
...
impl Div for Bitangent {
	...
	fn div(self, rhs :Bitangent) -> Bitangent {
		...
		(slef.bitangent.1 / rhs.bitangent.1),(self.bitangent.2 / rhs.bitangent.2))}
	}
}
...
impl Div for Tangent {
	...
	fn div(self, rhs: Tangent) -> Tangent {
		...
		(slef.tangent.1 / rhs.tangent.1),(self.tangent.2 / rhs.tangent.2))}
	}
}
...
#[allow(dead_code)]
pub fn cross_product(first:[f32; 3], second:[f32; 3]){
